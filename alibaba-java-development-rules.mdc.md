...
基于本项目（中小微企业信用报告服务）的实际情况，额外强调：

1. 本项目使用 Java 8 + Spring Boot 2.7.7，注意 JDK 版本兼容性。
2. 本项目使用 Kafka 消息队列，Listener 中必须做好异常处理和幂等性控制。
3. 本项目涉及 PDF 生成、Word 转 PDF 等文件操作，注意资源流的关闭（使用 `try-with-resources`）。
4. 本项目使用 SFTP 文件传输，注意连接池的释放和超时配置。
5. 本项目使用 ESB 服务调用，注意超时设置和异常兜底。
6. 本项目为多模块 Maven 工程，注意模块间依赖的合理性和 API 模块的干净性。
